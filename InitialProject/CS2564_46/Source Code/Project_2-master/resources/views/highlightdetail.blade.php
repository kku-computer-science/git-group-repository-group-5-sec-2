...
<body>
    ...
    <div class="container mt-3 px">
        <div>
            ...
            <div class="mt-2">
                <p class="info" id="creationInfo">{{trans('message.publish')}} : {{$highlight->created_at->format('Y-m-d H:i')}}   {{trans('message.by')}} {{$highlight->creator}}</p>
                    ...
                    <div class="row">
                        @foreach($highlight->images as $image)
                            ...
                            <div class="col-sm-12 col-md-6 col-lg-2 p-2 img-container">
                                <img src="{{$image->image_path}}" id="image-{{$image->id}}" class="image" data-bs-toggle="modal" data-bs-target="#imageModal{{ $image->id }}" alt="highlight image">
                            </div>

                            <!-- modal -->
                            <div class="modal fade overflow-hidden blur" id="imageModal{{ $image->id }}" tabindex="-1" aria-labelledby="imageModalLabel{{ $image->id }}" aria-hidden="true">
